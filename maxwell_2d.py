...
def flatten_hamiltionian(i, j):
    ...
    if j - 1 >= 0:
        row[0][j - 1][i][2] = -1
    row[0] /= electric_coeff_sqrt * magnetic_coeff_sqrt * 2 * mesh_step

    if i + 1 <= grid_size[0]:
        row[1][j][i + 1][2] = 1
    if i - 1 >= 0:
        row[1][j][i - 1][2] = -1
    row2 /= electric_coeff_sqrt * magnetic_coeff_sqrt * 2 * mesh_step
    ...
    if i - 1 >= 0:
        row[2][j][i - 1][1] = 1
    row[2] /= electric_coeff_sqrt * magnetic_coeff_sqrt * 2 * mesh_step
